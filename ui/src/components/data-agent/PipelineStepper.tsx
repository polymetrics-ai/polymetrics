...
export const PipelineStepper: React.FC<PipelineStepperProps> = ({ stepper, pipelineData }) => {
    ...
    const filteredSteps = stepper.all.filter(
        (step) =>
            Object.values(actionStepMap).includes(step.id) &&
            pipelineData?.actions?.some(
                (action) =>
                    action.action_type ===
                    Object.keys(actionStepMap).find((key) => actionStepMap[key] === step.id)
            )
    );
                            ...
                            <React.Fragment key={step.id}>
                                <li className="flex items-start gap-4">
                                    <div className="flex flex-col items-center">
                                        <div
                                            className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center bg-white border-2 border-emerald-500${
                                                step.id
                                                    ? 'bg-white border-2 border-emerald-500'
                                                    : index < filteredSteps.indexOf(stepper.current)
                                    ...
                                    <div className="flex-1 pt-1 pb-8">
                                        ...
                                        {step.id === 'connector-selected' && (
                                            <ConnectorIdentified
                                                connectors={
                                                    pipelineData?.actions?.find(
                                                        (a) =>
                                                            a.action_type === 'connector_selection'
                                                    )?.data.connectors
                                                }
                                            />
                                        )}
                                        {step.id === 'connection-created' &&
                                            pipelineData?.actions
                                                ?.find(
                                                    (a) => a.action_type === 'connection_creation'
                                                )
                                        ...
                                        {step.id === 'sync-initialized' && (
                                            <SyncInitiated pipeline_data={pipelineData} />
                                        )}
                                        {step.id === 'query-generated' && (
                                            <QueryBlock
                                                query={{
                                                    sql: pipelineData?.actions?.find(
                                                        (a) => a.action_type === 'query_generation'
                                                    )?.data.query,
                                                    explanation: pipelineData?.actions?.find(
                                                        (a) => a.action_type === 'query_generation'
                                                    )?.data.explanation
                                                }}
                                            />
                                        ...
                                        {step.id === 'query-executed' && (
                                            <DataPresented
                                                queryData={
                                                    pipelineData?.actions?.find(
                                                        (a) => a.action_type === 'query_execution'
                                                    )?.data.query_data
                                                }
                                                limit={
                                                    pipelineData?.actions?.find(
